...
def generate_local_fallback_insights(
    ...
    insights += "৪. ক্রেতাদের চাহিদা বুঝতে নিয়মিত লোকাল ডাটাবেজে বেচাকেনার হিসাব লিখে রাখুন।"
...
async def generate_insight(request: InsightRequest) -> InsightResponse:
    try:
        # Extract metadata with string safe fallbacks
        shop_name = (request.shop_name or "আমার জেনারেল স্টোর").strip()
        period = (request.period or "last_30_days").strip()

        logger.info(f"Processing insight request for shop: '{shop_name}' over period: '{period}'")

        # Safely get collections list
        sales_list = request.sales or []
        stock_list = request.stock or []

        # 1. Group sales by product to aggregate revenue and quantity safely
        ...
        total_qty = sum((item.qty if item.qty is not None else 0.0) for item in sales_list)

        # 2. Identify top selling product by quantity sold
        if product_sales_qty:
            top_product = max(product_sales_qty, key=lambda k: product_sales_qty[k])
        else:
            top_product = "None"

        # 3. Product ranking by revenue (sorting descending)
        ...
        # 4. Group stock by product to handle any duplicate entries safely
        ...
        # 5. Low stock products (remaining < 10)
        low_stock_products = [p for p, rem in product_stock_levels.items() if rem < 10]

        # 6. Dead stock detection (remaining > 0 but has 0 sales in the period)
        dead_stock_products = []
        ...
        # 7. Create computed summary representation for caching key
        summary_dict = {
        ...
        # 8. Query Cache to see if we have a fresh generated response for this metrics signature
        ...
        # 9. Cache missed. Formulate compact prompts for the Groq call
        ...
        # 10. Call Groq with multiple layers of fallback to avoid rate limits or service denial
        ...
        try:
            ...
            try:
                # Second attempt: fallback to settings model
                ...
                # Third attempt: Local rule-based fallback generator
        ...
        # 11. Update Cache with the newly computed insights
